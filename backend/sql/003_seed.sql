...
-- Run AFTER 001_schema.sql, 002_functions.sql, and after the user
-- accounts have been created by scripts/seed.py, which hashes the
-- passwords. This file attaches staff and clinical data to them.
-- ================================================================
...
insert into staff (id, staff_no, full_name, role, department, on_duty) values
  ((select id from users where email='doctor@medicare.com'),    'ST-001','Dr. Michael Chen',   'doctor',       'Cardiology',   true),
  ((select id from users where email='nurse@medicare.com'),     'ST-002','Grace Adjei',        'nurse',        'Outpatient',   true),
  ((select id from users where email='reception@medicare.com'), 'ST-003','Akosua Darko',       'receptionist', 'Front Desk',   true),
  ((select id from users where email='lab@medicare.com'),       'ST-004','Kwesi Antwi',        'lab',          'Laboratory',   true),
  ((select id from users where email='radiology@medicare.com'), 'ST-005','Dr. Omar Haddad',    'radiology',    'Radiology',    true),
  ((select id from users where email='pharmacy@medicare.com'),  'ST-006','Yaa Frimpong',       'pharmacist',   'Pharmacy',     true),
  ((select id from users where email='cashier@medicare.com'),   'ST-007','Kofi Owusu',         'cashier',      'Accounts',     true),
  ((select id from users where email='admin@medicare.com'),     'ST-008','Nana Ampofo',        'admin',        'Administration', true);

-- Roster doctors. Schedulable staff with no password, so they appear in
-- the booking roster but nobody can sign in as them.
insert into staff (id, staff_no, full_name, role, department, on_duty) values
  ((select id from users where email='emily.parker@medicare.com'),  'ST-009','Dr. Emily Parker',   'doctor','Neurology',    true),
  ((select id from users where email='lisa.thompson@medicare.com'), 'ST-010','Dr. Lisa Thompson',  'doctor','Dermatology',  true),
  ((select id from users where email='james.wilson@medicare.com'),  'ST-011','Dr. James Wilson',   'doctor','Orthopaedics', false);

-- ---------- 2. patients ----------
-- Patient 1 is the mobile app account and is linked to a user.

insert into patients (mrn, user_id, full_name, age, sex, phone, blood_group, allergies, conditions, insurance, last_visit) values
  ('PT-20481', (select id from users where email='patient@medicare.com'),
...
declare
  ...
  -- A flat list of eight condition slots, empty string meaning none.
...
insert into appointments (mrn, doctor_id, specialty, appt_date, appt_time, appt_type, status) values
  ('PT-20481',(select id from staff where staff_no='ST-001'),'Cardiology', current_date,          '09:00','Follow-up','confirmed'),
  ('PT-20518',(select id from staff where staff_no='ST-009'),'Neurology',  current_date,          '10:30','Consultation','confirmed'),
  ('PT-20524',(select id from staff where staff_no='ST-001'),'Cardiology', current_date + 1,      '14:00','Consultation','confirmed'),
  ('PT-20536',(select id from staff where staff_no='ST-010'),'Dermatology',current_date + 2,      '11:00','Review','confirmed');
