-- ============================================================
-- MediCare+ HMS — PASTE 1 of 2: schema and row-level security
-- ============================================================
-- Paste this whole file into the Supabase SQL editor and run it.
-- Then run scripts/create-users.mjs, THEN paste 2.
--
-- Order matters: the seed in paste 2 attaches staff rows to auth
-- users by email, so those users must exist before it runs.
-- ============================================================
...
-- Added `security definer`. Without it, verifying a result fails with
--   ERROR: new row violates row-level security policy for table "notifications"
-- because rls-policies.sql enables row-level security on notifications and
-- grants a select and an update policy but no INSERT policy to anybody. The
-- technician doing the verifying therefore cannot write the notification the
-- release depends on, so the single most important clinical rule in the
-- system — nothing reaches the doctor or the patient until a technician
-- verifies — could never fire.
--
-- This follows the pattern the handoff already uses and states explicitly
-- for invoice_lines: "Lines are written by the add_invoice_line function,
-- which is security definer, so no direct insert policy is granted." The
-- same reasoning applies to system-generated notifications. write_audit()
-- and add_invoice_line() are security definer for this exact reason; this
-- trigger was the one that got missed.
